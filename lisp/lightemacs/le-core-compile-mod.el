...
(defun lightemacs--byte-compile-if-outdated (el-file &optional no-error)
  "Byte-compile EL-FILE into a .elc if it is missing or outdated.

If the corresponding .elc file does not exist or is older than EL-FILE, this
function attempts to compile EL-FILE. Compilation warnings and errors are
suppressed, and verbose messages are displayed if `lightemacs-verbose` is
non-nil.

NO-ERROR, if non-nil, suppresses raising an error when EL-FILE cannot be found.
Returns t if the file is up to date or successfully compiled, nil or the error
object otherwise."
