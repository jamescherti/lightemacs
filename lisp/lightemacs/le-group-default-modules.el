...
(lightemacs-load-modules
 '(;; Hides or abbreviates mode indicators in the Emacs mode line for a cleaner
   ;; display
   ;; TODO make this module diminish all others instead of using :diminish
   le-diminish
   ...
   ;; Gcmh optimizes Emacs’ garbage collection behavior by adjusting the garbage
   ;; collection threshold dynamically. Instead of collecting memory frequently
   ;; during normal editing, gcmh increases the threshold while Emacs is idle,
   ;; reducing interruptions and improving perceived performance.
   le-gcmh
